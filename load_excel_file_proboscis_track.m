function load_excel_file_proboscis_track(files)
% loads -csv files with the animal data in the following format: X, Y
% coordinate of the following points in this order:
% proboscis, head, thorax, [there can be other body points, but those won't
% be analysed], outline of the flower, pattern 1, pattern 2
...
outline(isnan(outline(:,1)),:)=[];%somehow I only get the matrix to stay in shape this way
...
if size(num,2)>indOutline+3
    ...
    pattern2(indnan(:,1),:)=[];%somehow I only get the matrix to stay in shape this way
else
    pattern2=[nan nan];
end
...
%DO NOT DELETE ANY NAN ENTRIES FROM TRACKING VARIABLES OR THEY GET OUT OF
%SYNC!!!!
...
save(outputname,'pattern2','pattern','prob','outline','thorax','head','dataMatrix','labels');
