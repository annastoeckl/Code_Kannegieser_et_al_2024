% This script loads the tracked data, adds together data from multiple flights, and rotates the pattern to align with the cardinal coordinataes
clear all
close all
...
allPBaseR=[];%probiscis base coordinates
allLAntBaseR=[];%antenna base left coordinates
allRAntBaseR=[];%antenna base right coordinates
...
for j=1:length(filenames)
    ...
    if sum(isnan(pattern(:)))~=0 || ~isempty(indCircle)
        ...
        headR=head-midpoint;
        PBaseR=probBase-midpoint;
        LAntBaseR=antBaseL-midpoint;
        RAntBaseR=antBaseR-midpoint;
        ...
        allPBaseR=[allPBaseR;PBaseR];
        allLAntBaseR=[allLAntBaseR;LAntBaseR];
        allRAntBaseR=[allRAntBaseR;RAntBaseR];
        allR=nan;
        ...
        headR=[R*head']';
        PBaseR=[R*probBase']';
        LAntBaseR=[R*antBaseL']';
        RAntBaseR=[R*antBaseR']';
        ...
        if check(1)>check(2)
            ...
            headR=[R90*headR']';
            PBaseR=[R90*PBaseR']';
            LAntBaseR=[R90*LAntBaseR']';
            RAntBaseR=[R90*RAntBaseR']';
            for u=1:size(dataMatrix,2)/2
        ...
        PBaseR=PBaseR-midpoint;
        LAntBaseR=LAntBaseR-midpoint;
        RAntBaseR=RAntBaseR-midpoint;
    ...
    %didn't do this for the head tracked points (prob and ant base),
    %becaues these were tracked by hand
    ...
    plot(PBaseR(:,1),PBaseR(:,2),'*','color',cols(j,:));
    plot([patternR(:,1);patternR(1,1)],[patternR(:,2);patternR(1,2)],'-','color',cols(j,:));
    plot([pattern2R(:,1);pattern2R(1,1)],[pattern2R(:,2);pattern2R(1,2)],'-','color',cols(j,:));
    plot(xunitC,yunitC,'-','color',cols(j,:));
    xlim([-300 300]);ylim([-300 300]);
    ...
    plot(LAntBaseR(:,1),LAntBaseR(:,2),'o','color',cols(j,:));hold on;
    plot([patternR(:,1);patternR(1,1)],[patternR(:,2);patternR(1,2)],'-','color',cols(j,:));
    plot([pattern2R(:,1);pattern2R(1,1)],[pattern2R(:,2);pattern2R(1,2)],'-','color',cols(j,:));
    plot(xunitC,yunitC,'-','color',cols(j,:));
    xlim([-300 300]);ylim([-300 300]);
    ...
    plot(RAntBaseR(:,1),RAntBaseR(:,2),'*','color',cols(j,:));hold on;
    plot([patternR(:,1);patternR(1,1)],[patternR(:,2);patternR(1,2)],'-','color',cols(j,:));
    plot([pattern2R(:,1);pattern2R(1,1)],[pattern2R(:,2);pattern2R(1,2)],'-','color',cols(j,:));
    plot(xunitC,yunitC,'-','color',cols(j,:));
    xlim([-300 300]);ylim([-300 300]);
    ...
    clear dataR probR headR thoraxR PBaseR LAntBaseR RAntBaseR
end
...
save(['allRotData_',filenames{1}(indStart+1:end)],'allprobR','allThoraxR','allHeadR','allPBaseR','allLAntBaseR','allRAntBaseR','patternR','pattern2R','outlineR','allR','allDataR','labels','timeBouts')